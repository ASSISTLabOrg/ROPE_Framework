...
ROPE::ROPE(
    ...
    const std::string& decoder_device
) {
    ...
    std::cout << "  backend : LibTorch  device=" << decoder_device << "\n";
    decoder_model_ = make_model(
        jpath(exported_dir, "coae_decoder.pt"),
        ModelBackend::LibTorch,
        /*intra_op_threads=*/intra_threads_decoder,
        /*use_dnnl=*/false,
        decoder_device
    );
#else
    std::cout << "  backend : ONNX Runtime  threads=" << intra_threads_decoder << "\n";
    decoder_model_ = make_model(
