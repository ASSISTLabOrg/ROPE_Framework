/*
 * Rope.cs — C# binding for the ROPE framework (.NET Framework 4.8).
 *
...
namespace RopeFramework
{
    ...
    public sealed unsafe class Rope : IDisposable
    {
            ...
            string configPath = null)
        {
            ...
            if (libPath == null) libPath = ResolveLibPath(root);
            if (exePath == null) exePath = ResolveExePath(root);
        ...
        private static string PackageRoot()
        {
            ...
            return asmDir != null ? Path.GetFullPath(Path.Combine(asmDir, "..")) : ".";
        }
        ...
        private static string ResolveLibPath(string root)
        {
            ...
            if (RuntimeInformation.IsOSPlatform(OSPlatform.Windows))
                candidates = new string[] { Path.Combine(root, "bin", "rope.dll") };
            else if (RuntimeInformation.IsOSPlatform(OSPlatform.OSX))
                candidates = new string[] { Path.Combine(root, "lib", "librope.dylib") };
            else
        ...
        private static string ResolveExePath(string root)
        {
            string[] candidates;
            if (RuntimeInformation.IsOSPlatform(OSPlatform.Windows))
                candidates = new string[] { Path.Combine(root, "bin", "rope.exe") };
            else
                candidates = new string[]
                {
